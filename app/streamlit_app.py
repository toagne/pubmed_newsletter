...
def handle_edit_research_interests():
	...
	with st.form(key='edit_research_interests_form'):
		user = db.get_user(st.session_state.pending_email)
		email = user[1]
		st.text(email)
		query = st.text_area("💬 Your Query", value=user[2] if user[2] else None, placeholder="Describe your query here..." if not user[2] else None, )
		all_journals = db.get_all_journals()
		journals = st.multiselect("Journals", all_journals, filter_mode="contains", default=user[3].split(journals_join_split) if user[3] else None)
		num_papers = st.slider("Number of papers to receive each month", min_value=10, max_value=100, value=user[4] if user[4] else 20, step=10)
